public class Grass
{
    public char organismChar;
    public int foodChainLevel;
    public boolean carnivorousType;

    public Grass(){
		organismChar = 'g'; 	//a 'g' on the matrix grid will represent a grass there
		foodChainLevel = 0; 		//level 0 of the food chain, meaning it does not eat anything, but is eaten by higher levels
		carnivorousType = false;	//whether it is a meat-eater or not
    }

    public int retrieveFoodLevel(){
		return foodChainLevel;
    }

    public boolean retrieveOrganismType(){
		return carnivorousType;
    }
	...
	switch (generateXCoord){
		...
		System.out.println("\nThe coordinates of the grass are: X: " + generateXCoord + ", Y: " + generateYCoord + ".");
    }
}